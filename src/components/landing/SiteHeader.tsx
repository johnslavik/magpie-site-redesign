...
export function SiteHeader({ search = false, currentPath = "" }: { search?: boolean; currentPath?: string }) {
  ...
  const themeToggle = <button className="theme-toggle" type="button" aria-label="Switch to dark mode" aria-pressed="false"><Moon className="theme-moon" size={18} aria-hidden="true" /><Sun className="theme-sun" size={18} aria-hidden="true" /></button>;
