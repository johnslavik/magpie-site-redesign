...
export default function AirflowCharts() {
 ...
 return <div className="airflow-charts">
   ...
   <div className="chart-scroll"><svg viewBox={`0 0 ${width} ${height}`} className="airflow-chart" role="img" aria-label={`${view.title}, ${period}, from ${dataset.buckets[0]} to ${dataset.buckets[count-1]}. Last period is incomplete. Exact values available in the data table below.`}>
    ...
    {dataset.buckets.map((bucket,index)=><g key={bucket}>{(period==='quarterly'||index%6===0||index===count-1)&&<text x={x(index)} y={height-18} textAnchor="middle" className="chart-label">{axisLabel(bucket)}{index===count-1?'*':''}</text>}<rect x={x(index)-plotWidth/count/2} y={top} width={plotWidth/count} height={plotHeight} fill="transparent" tabIndex={0} role="button" aria-label={`${bucket}: ${traces.map(t=>`${friendly(t.label)} ${t.values[index]??'not available'}`).join('; ')}`} onMouseEnter={()=>setActive(index)} onMouseLeave={()=>setActive(null)} onFocus={()=>setActive(index)} onBlur={()=>setActive(null)} onClick={()=>setActive(active===index?null:index)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();setActive(active===index?null:index);}}}/></g>)}
